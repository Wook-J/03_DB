...
/* VIEW 를 생성하기 위해서는 권한이 필요함!!! */

-- (관리자 계정 접속) < >안에 SYS로 바뀐거 확인!
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;
--> 계정명이 언급되는 상황에서는 위 구문을 반드시 실행!

-- VIEW 생성권한 부여
GRANT CREATE VIEW TO kh_jwj;
-- (여기까지 관리자 계정) 다시 < >안에 kh_jwj으로 바꾸기!


-- SQL Error [1031] [42000]: ORA-01031: 권한이 불충분합니다(위 관리자계정 코드 미실행 시)
-- VIEW 생성구문 기초 문법
...
--> 덮어쓰는 방법(OR REPLACE 구문 추가)으로 해결가능!
...
-- 원본 테이블에서 확인
SELECT * FROM DEPT_COPY2;
-- 10행에 (DO, [NULL], L2)가 추가되어 있음...!

-- VIEW에 INSERT를 수행했는데, 
-- VIEW 생성 시 사용한 원본 테이블에 값이 INSERT 되어있음!

-- 그러나 모든 컬럼 값이 INSERT 된 것이 아니라
-- VIEW를 생성할 때 사용된 컬럼에만 데이터가 삽입되었고
-- 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어감!
--> NULL은 DB의 무결성을 약하게 만드는 주요 원인이어서
 ...
 * ** 사용법 **
 *
 * 1) 시퀀스명.NEXTVAL : 다음 시퀀스 번호를 얻어옴.(맨처음에 한번은 불러야함!)
...
CREATE TABLE TB_TEST (
	TEST_N0 NUMBER PRIMARY KEY,		-- 이름을 잘못만들었네,,, 마지막이 알파벳 O여야 하는데 숫자 0으로 만듦...
...
--> NEXTVAL를 호출한 적이 없어서 오류발생

-- 해결방법 : NEXTVAL 호출하면 해결
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 시퀀스 생성 후 첫 NEXTVAL == START WITH 값인 100이 호출됨

SELECT SEQ_TEST_NO.CURRVAL FROM DUAL;
-- 위에서 NEXTVAL 호출하여 100 존재하므로, CURRVAL 하면 100이 호출됨

-- NEXTVAL를 호출할 때마다 INCREMENT BY에 작성된 수 만큼 증가하는지 확인
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
...
/* UPDATE 에서 시퀀스 사용하기 */
-- '짱구'의 PK 컬럼값을 SEQ_TEST_NO 시퀀스의 다음 생성 값으로 변경하기
UPDATE TB_TEST SET TEST_N0 = SEQ_TEST_NO.NEXTVAL WHERE TEST_NAME = '짱구';
-- SQL Error [8004] [72000]: ORA-08004: 시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다

-- 짱구의 TEST_N0 값을 150까지 증가시키고 나서 또 한번 SEQ_TEST_NO.NEXTVAL 호출
 ...
 * [작성법]
  ...
  [INCREMENT BY 숫자] -- 다음 값에 대한 증가치, 생략하면 자동 1이 기본 (숫자에 음수도 가능)
  [MAXVALUE 숫자 | NOMAXVALUE] -- 발생시킬 최대값 지정 (10의 27승 -1)
  [MINVALUE 숫자 | NOMINVALUE] -- 최소값 지정 (-10의 26승)
  [CYCLE | NOCYCLE] -- 값 순환 여부 지정
  [CACHE 바이트크기 | NOCACHE] -- 캐쉬메모리 기본값은 20바이트, 최소값은 2바이트
...
SELECT COUNT(*) FROM TB_IDX_TEST;		-- 100만개 행이 삽입되었는지 확인용
...
/* 인덱스 X */
-- TEST_ID 가 'TEST500000'인 행 조회
SELECT * FROM TB_IDX_TEST WHERE TEST_ID = 'TEST500000';		-- 0.014s
