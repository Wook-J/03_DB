...
-- EMPLOYEE 테이블에서 성별(남/여)과 각 성별별 인원수, 급여 합을
-- 인원 수 오름차순으로 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') "성별",
COUNT(*) "인원 수",
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여')		-- 아직 별칭 사용 불가능
ORDER BY "인원 수";		-- 별칭 사용 가능( SELECT절 해석 완료되었으므로! )
...
-- * WHERE 절과 GROUP BY 절 혼합하여 사용하기 *
--> WHERE 절은 각 컬럼값에 대한 조건
--> HAVING 절은 GROUP에 대한 조건
...
SELECT * FROM EMPLOYEE;
------------------------------------------------------------
-- * 여러 칼럼을 묶어서 그룹으로 지정 가능 -> 그룹 내 그룹이 가능함!! *
...
-- EMPLOYEE 테이블에서 부서별 같은 직급인 사원의 인원 수를 조회
...
-- SQL Error [979] [42000]: ORA-00979: GROUP BY 표현식이 아닙니다.(JOB_CODE를 안쓰는 경우)

--------------------------------------------------------------------------------
-- * HAVING 절 : 그룹함수로 구해 올 그룹에 대한 조건을 설정할 때 사용
-- HAVING 컬럼명 | 함수식 비교연산자 비교값

-- EMPLOYEE 테이블에서 부서별 평균급여가 300만원 이상인 부서의
-- 부서코드, 평균 급여 조회
-- 부서코드 오름차순
...
-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 그룹의
-- 직급코드, 인원수 조회(직급코드 오름차순)
SELECT JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT(*) <= 5	-- HAVING 절에는 그룹함수가 반드시 작성됨!
ORDER BY 1;		-- 컬럼명 대신 N번째 컬럼위치 써도됨!

---------------------------------------------------------------------------------
-- 집계 함수 (ROLLUP, CUBE)
-- 그룹별 산출 결과 값의 집계를 계산하는 함수
-- (그룹별로 중간 집계 결과를 추가하여 RESULT SET에 보여줌)
...
WHERE DEPT_CODE = 'D5'
--UNION					-- 합집합 부분(12개)
--INTERSECT				-- 교집합 부분(2개) (대북혼, 심봉선)
--UNION ALL				-- 중복부분도 세주기(14개)
MINUS					-- 선행 SELECT 결과에서 다음 SELECT 결과와 겹치는 부분제외
-- 급여가 300만원 초과인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000;

-- 위에 2개를 하나의 RESULT SET으로 나타내기
-- 위 SELECT 문 가운데 UNION, INTERSECT ... 추가!(앞 SELECT 문에서 ; 제거)

-- (주의사항!) 집합 연산자를 사용하기 위한 SELECT 문들은
-- 조회하는 컬럼의 타입, 개수가 모두 동일해야 함!!

SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '안녕'
FROM EMPLOYEE
WHERE SALARY > 3000000;
-- SQL Error [1789] [42000]: ORA-01789: 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다.(SELECT 컬럼 개수 다를 시)
-- 타입맞으면 되서 SALARY 대신에 1을 넣어도 결과는 나옴!
-- SQL Error [1790] [42000]: ORA-01790: 대응하는 식과 같은 데이터 유형이어야 합니다(SELECT 컬럼 개수 같은데 타입 다를 시)
